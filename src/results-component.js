...
export function makeWinnerResultsArea(candidates) {
    const html = `
        <section>
            ...
            <h3><a href="candidate-detail.html#f=${candidates[0].firstName}&l=${candidates[0].lastName}">Candidate Detail</a></h3>
        </section>`;
...
export function makeRunnersUpArea(candidates) {
    const html = `
        <section>
            ...
            <p><a href="candidate-detail.html#f=${candidates[1].firstName}&l=${candidates[1].lastName}">Candidate Detail</a></p>
            ...
            <p><a href="candidate-detail.html#f=${candidates[2].firstName}&l=${candidates[2].lastName}">Candidate Detail</a></p>
        </section>`;
...
export function makeOthersArea(candidate) {
    const html = `
        <li>
            <p>${candidate.lastName}: ${candidate.debateScore} points, <a href="candidate-detail.html#f=${candidate.firstName}&l=${candidate.lastName}">Candidate Detail</a></p>
        </li>`;
    const template = document.createElement('template');
    template.innerHTML = html;
    return template.content;
